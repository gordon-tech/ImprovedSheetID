...
from numba import njit, float64, prange, int64, jit
...
def getPenalizedCombFilter(linesep: float):
...
#         np.testing.assert_array_equal(fftconvolve(rowMedians, filt, mode='same'), convolve2d(rowMedians, filt, mode='same'))
        responses[i, :, :] = fftconvolve(rowMedians, filt, mode='same')
...
@njit
def get_tuples(cluster_centers, templateH, templateW):
    bboxes = []
    for i in range(len(cluster_centers)):
        r, c = cluster_centers[i]
...
def extractNotesFromChordBlock(region, templateSpecs):
    ...
    bboxes = get_tuples(kmeans.cluster_centers_, templateH, templateW)
    return bboxes
